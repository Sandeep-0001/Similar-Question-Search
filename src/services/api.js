...
// Direct LeetCode GraphQL call via proxy
// - In Vite dev, we use the /leetcode/graphql dev proxy
// - In production (Vercel), we hit our serverless API route /api/leetcode
const LEETCODE_ENDPOINT = import.meta.env.DEV
  ? '/leetcode/graphql'
  : (import.meta.env.VITE_LEETCODE_PROXY || '/api/leetcode');
...
export const fetchSimilarFromLeetCode = async (titleSlug) => {
  ...
  const resp = await fetch(LEETCODE_ENDPOINT, {
    method: 'POST',
